...
// BEGINNING SEMPERFI TESTIMONIALS 
    $( "#ccScrollTestimonials" ).on( "click", "#ccScrollTestimonials-next", function(event) {
        
        event.preventDefault();

        var testimonials_list = $('.testimonials_list');
        var next_link = $('a#ccScrollTestimonials-next');
        var loadingHtml = '<p class="text-center" id="loading"><i class="fa fa-spinner fa-spin thickpad-topbottom"></i> ' + $('#lang_loading').text() + '&hellip;<p>';
        ...
        $.ajax({
            ...
            complete: function(returned) {
                ...
                var page = returned.responseText;
                var list = $('.testimonials_list div', page);
                var next = $('a#ccScrollTestimonials-next', page);
                
                testimonials_list.append(list);
                $(next_link).replaceWith(next);
            }
        });
    });
