...
<div id="element-reviews">
   ...
   <div id="review_read">
      {if $REVIEWS}
     <p class="pagination_top"><span class="pagination">{if isset($PAGINATION)}{$PAGINATION}{/if}</span>{$LANG.catalogue.average_rating}: <strong>{if {$REVIEW_AVERAGE}==0} Not Rated Yet{else}{$REVIEW_AVERAGE}</strong>{/if} </p>
   ...
   <div id="review_write" class="hide">
      ...
      <form action="{$VAL_SELF}#reviews_write" id="review_form" method="post">
         <div class="panel">
            ...
            {else}
            <div class="row">
               <div class="small-12 columns"><label for="rev_name"><span class="redbold">{$LANG.common.name}</span></label><input id="rev_name" type="text" name="review[name]" value="{$WRITE.name}" placeholder="{$LANG.common.name} {$LANG.form.required}" required></div>
            </div>
            <div class="row">
               <div class="small-12 columns"><label for="rev_email"><span class="redbold">{$LANG.common.email}</span></label><input id="rev_email" type="text" name="review[email]" value="{$WRITE.email}" placeholder="{$LANG.common.email} {$LANG.form.required}" required></div>
            </div>
            {/if}
            <div class="row">
               <div class="small-12 columns" id="review_stars">
                 <label for="rating"><span class="redbold">{$LANG.documents.rating}  </span>{$LANG.form.required}</label>
            ...
            <div class="row">
               <div class="small-12 columns"><label for="rev_title" class="inline"><span class="redbold">{$LANG.catalogue.review_title}</span></label><input id="rev_title" type="text" name="review[title]" value="{$WRITE.title}" placeholder="{$LANG.catalogue.review_title} {$LANG.form.required}" required></div>
            </div>
            <div class="row">
               <div class="small-12 columns"><label for="rev_review" class="return"><span class="redbold">{$LANG.catalogue.review}</span></label><textarea id="rev_review" name="review[review]" placeholder="{$LANG.catalogue.review} {$LANG.form.required}" required>{$WRITE.review}</textarea></div>
            </div>
