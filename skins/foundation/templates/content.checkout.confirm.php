...
<div id="checkout_register_form"{if !empty($BILLING.line1)} class="hide"{/if}>
  ...
  <p><span class=redbold>{$LANG.account.already_registered} <a href="#" id="checkout_login">{$LANG.account.log_in}</a></span></p>
   <h3>{$LANG.account.contact_details}</h3>
   <div class="row">
   ...
   <div class="row">
      <div class="small-12 large-8 columns"><label for="addr_company" class="show-for-medium-up">{$LANG.address.company_name}</label><input type="text" name="billing[company_name]" id="addr_company"  value="{$BILLING.company_name}" placeholder="{$LANG.address.company_name}" autocomplete="organization" (or leave blank)></div>
   </div>
   <address>
      <div class="row">
         <div class="small-12 large-8 columns"><label for="addr_line1" class="show-for-medium-up">{$LANG.address.line1}</label><input type="text" name="billing[line1]" id="addr_line1"  required value="{$BILLING.line1}" placeholder="{if $ADDRESS_LOOKUP}{$LANG.address.address_lookup}{else}{$LANG.address.line1} {$LANG.form.required}{/if}" autocomplete="off" autocorrect="off" class="address_lookup"></div>
      </div>
      {if $ADDRESS_LOOKUP}
      ...
      <div class="row">
         <div class="small-12 large-8 columns"><label for="addr_line2" class="show-for-medium-up">{$LANG.address.line2}</label><input type="text" name="billing[line2]" id="addr_line2"  value="{$BILLING.line2}" placeholder="{$LANG.address.line2}" autocomplete="address-line2" (use line if needed)></div>
      </div>
      <div class="row">
      ...
      <div class="row">
         <div class="small-12 large-8 columns"><label for="addr_postcode" class="show-for-medium-up">{$LANG.address.postcode}</label><input type="text" name="billing[postcode]" id="addr_postcode"  class="uppercase required" value="{$BILLING.postcode}" placeholder="{$LANG.address.postcode}   (e.g. 35178, AB 593) {$LANG.form.required}" autocomplete="postal-code"></div>
      </div>
      <div class="row">
...
<div class="hide" id="address_delivery">
   ...
   <address>
      ...
      <div class="row">
         <div class="small-12 large-8 columns"><label for="del_postcode" class="show-for-medium-up">{$LANG.address.postcode}</label><input type="text" name="delivery[postcode]" id="del_postcode"  class="uppercase required" value="{$DELIVERY.postcode}" placeholder="{$LANG.address.postcode}  (e.g. 35178, AB 593) {$LANG.form.required}" autocomplete="postal-code"></div>
      </div>
      <div class="row">
...
<div id="account-reg">
   ...
   <div class="row">
      <div class="small-12 large-8 columns"><label for="reg_password" class="show-for-medium-up">{$LANG.account.password}</label><input type="password" autocomplete="off" name="password" id="reg_password"  required  placeholder="{$LANG.account.password}  (at least 6 characters) {$LANG.form.required}" autocomplete="new-password"></div>
   </div>
   <div class="row">
...
<!--End mc_embed_signup--> 
 {* MODSINDEX RECAPTCHA  {$DISPLAY_RECAPTCHA}  *}
