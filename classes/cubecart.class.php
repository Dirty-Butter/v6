...
class Cubecart {
	...
	const ORDER_FAILED   = 5; /*I USE THIS FOR UNPAID */
	...
	public function getDocument($doc_id = null, $homepage = false) {
		if (is_numeric($doc_id) || $homepage) {
			...
			if ($GLOBALS['user']->is() || $GLOBALS['config']->get('config','sfws_site_doc_manager_status') == 0) {
			$where  = ($homepage) ? array('doc_home' => true, 'doc_status' => '1') : array('doc_id' => $doc_id, 'doc_status' => '1');
			} else {
				$where = ($homepage) ? array('doc_home' => true, 'doc_status' => '1') : array('doc_id' => $doc_id, 'doc_status' => '1', 'doc_access' => '0');
			}
	...
	public function loadPage() {
			...
			switch (strtolower($_GET['_g'])) {
				...
				case 'document':
					...
					if (($document = $this->getDocument($_GET['doc_id'])) !== false) {
						...
						//SEMPERFI DOCUMENT MANANGER REPLACEMENT BELOW$content = $GLOBALS['smarty']->fetch('templates/content.document.php');
						//$GLOBALS['smarty']->assign('PAGE_CONTENT', $content);
					//}
		$content = $GLOBALS['smarty']->fetch('templates/content.document.php');
					...
					break;
//END SEMPERFI DOCUMENT MANAGER REPLACEMENT
...
// BEGINNING SEMPERFI TESTIMONIALS 
				case 'testimonials':
					$GLOBALS['smarty']->assign('SECTION_NAME', 'testimonials');
					$this->_testimonials();
					break;
				case 'addtestimonial':
					$GLOBALS['smarty']->assign('SECTION_NAME', 'addtestimonial');
					$this->_addtestimonial();
					break;
				// END SEMPERFI TESTIMONIALS 
// Start SemperFi ASKABOUT Addition
				case 'askabout':
					$GLOBALS['smarty']->assign('SECTION_NAME', 'askabout');
					$this->_askabout();
				break;
				// End SemperFi ASKABOUT Addition
...
foreach ($GLOBALS['hooks']->load('class.cubecart.construct.confirm_recaptcha_check') as $hook) include $hook;
...
// Start SemperFi Addition ASKABOUT
	/**
	 * Display ask about a product page
	 */
	private function _askabout() {
		/* !Ask About A Product Form */
		$askabout_contact = $GLOBALS['config']->get('Contact_Form');
		if($GLOBALS['config']->get('config','ask_about_a_product_status') == 1){
			$GLOBALS['gui']->addBreadcrumb($GLOBALS['language']->askaboutaproduct['breadcrumb'], $GLOBALS['seo']->buildURL('askabout'));
			// Display form
			if(isset($_GET['product_id']) && is_numeric($_GET['product_id'])){
				$getproductquery = 'SELECT * FROM '.$GLOBALS['config']->get('config', 'dbprefix').'CubeCart_inventory WHERE product_id = '.$_GET['product_id'];
				$getproduct = $GLOBALS['db']->query($getproductquery);
				if ($getproduct) {
					$product_id	= $_GET['product_id'];
					$product_name = $getproduct[0]['name'];
					$product_code = $getproduct[0]['product_code'];
					$product_description = $getproduct[0]['description']; // I ADDED FROM 5.2.1 VERSION
					$askabout_description = sprintf($GLOBALS['language']->askaboutaproduct['product_valid_description'],$product_name); 
					if ((bool)$getproduct[0]['use_stock_level'] && $getproduct[0]['stock_level'] <= 0) {
						$GLOBALS['smarty']->assign('ASKABOUT_PRODUCT_DESCRIPTION_3', true);
					} else if ($getproduct[0]['sale_price'] > 34) {
						$GLOBALS['smarty']->assign('ASKABOUT_PRODUCT_DESCRIPTION_2', true);
					} else if ($getproduct[0]['price'] > 34) {
						$GLOBALS['smarty']->assign('ASKABOUT_PRODUCT_DESCRIPTION_2', true);
					}
					...
					$GLOBALS['smarty']->assign('ASKABOUT_PRODUCT_DESCRIPTION', $product_description); //  I ADDED FROM 5.2.1 VERSION
					...
					if($GLOBALS['config']->get('config', 'ask_about_a_product_departments') == 1){
						if (isset($askabout_contact['department']) && is_array($askabout_contact['department'])) {
							foreach ($askabout_contact['department'] as $key => $dept) {
								$dept['key']  = $key;
								$dept['selected'] = (isset($_POST['askabout']['department']) && $_POST['askabout']['department'] == $key) ? ' selected="selected"' : '';
								$vars['departments'][] = $dept;
							}
							$GLOBALS['smarty']->assign('ASKABOUT_DEPARTMENTS', $vars['departments']);
						}
					}
				} else {
					$askabout_description = $GLOBALS['language']->askaboutaproduct['product_invalid_description'];
					$GLOBALS['smarty']->assign('ASKABOUT_DESCRIPTION', $askabout_description);
					$GLOBALS['smarty']->assign('ASKABOUT', $askabout);
				}
			} else {
				$askabout_description = $GLOBALS['language']->askaboutaproduct['product_invalid_description'];
				$GLOBALS['smarty']->assign('ASKABOUT_DESCRIPTION', $askabout_description);
			...
			// Process Form
			if (isset($_POST['askabout']) && is_array($_POST['askabout'])) {
				$error	= false;
				// Validation
				$error = false;
				$required	= array('your_name', 'your_email', 'your_phone', 'department', 'message');
				foreach ($_POST['askabout'] as $key => $value) {
...
/*MODSINDEX RECAPTCHA MOD NEEDED FOR ASKABOUT
foreach ($GLOBALS['hooks']->load('class.cubecart.contact_us_recaptcha_check') as $hook) include $hook;
END MODSINDEX RECAPTCHA MOD FOR ASKABOUT*/
				if (!filter_var($_POST['askabout']['your_email'], FILTER_VALIDATE_EMAIL)) {
				...
				if (!$error) {
					...
					if($GLOBALS['config']->get('config', 'ask_about_a_product_departments') == 1){
						$department = '';
						if (isset($_POST['askabout']['department']) && is_array($askabout_contact['department'])) {
							$key		= (int)$_POST['askabout']['department'];
							$department = $askabout_contact['department'][$key]['name'];
							$email		= (!empty($askabout_contact['department'][$key]['email'])) ? $askabout_contact['department'][$key]['email'] : $email;
							$mail->addCC($email, $department);
						}
					}
					$data['sender_name']	= strip_tags($_POST['askabout']['your_name']);
					$data['sender_email']	= $_POST['askabout']['your_email'];
					$data['sender_phone']	= $_POST['askabout']['your_phone'];
					$data['department']		= $department;
					$data['product_name']	= strip_tags($_POST['askabout']['product_name']);
					$data['question']		= strip_tags($_POST['askabout']['message']);
					$data['product_id']		= $_POST['askabout']['product_id'];
					$data['product_code']	= $_POST['askabout']['product_code'];
					$product_url			= $GLOBALS['seo']->buildURL('prod', $_POST['askabout']['product_id']);
					$data['product_url']	= $product_url;
					$content = $mail->loadContent('admin.ask_about_a_product', $GLOBALS['language']->current(), $data);
					if (!empty($content)) {
						$mail->AddReplyTo($_POST['askabout']['your_email'], strip_tags($_POST['askabout']['your_name']));
						if($mail->sendEmail($GLOBALS['config']->get('config', 'email_address'), $content)){
							$notify_askaboutaproduct_success = sprintf($GLOBALS['language']->askaboutaproduct['notify_askaboutaproduct_success'],strip_tags($_POST['askabout']['product_name']));
							$GLOBALS['gui']->setNotify($notify_askaboutaproduct_success);
							//httpredir('index.php');
						} else {
							$GLOBALS['gui']->setError($GLOBALS['language']->askaboutaproduct['error_askaboutaproduct']);
						}
					} else {
						$GLOBALS['gui']->setError($GLOBALS['language']->askaboutaproduct['error_askaboutaproduct']);
					}
					...
					foreach($_POST['askabout'] as $key => $value) {
						$_POST['askabout'][$key] = htmlspecialchars($value);
					}
				/*MODSINDEX RECAPTCHA MOD NEW ATTEMPT
foreach ($GLOBALS['hooks']->load('class.cubecart.contact_us_recaptcha') as $hook) include $hook;
END MODSINDEX RECAPTCHA MOD*/
					$GLOBALS['smarty']->assign('MESSAGE', $_POST['askabout']);
				}
			}
			$content = $GLOBALS['smarty']->fetch('templates/content.askabout.php');
			$GLOBALS['smarty']->assign('PAGE_CONTENT', $content);
		} else {
			httpredir('index.php');
		}
	}
	// End SemperFi Addition ASKABOUT
...
// BEGINNING SEMPERFI TESTIMONIALS 
	...
	private function _testimonials() {
		...
		if($GLOBALS['config']->get('config','sfws_testimonials_page_status') == 1){

			$page = (isset($_REQUEST['page']) && !empty($_GET['page'])) ? $_REQUEST['page'] : 1;
			$per_page = $GLOBALS['gui']->itemsPerPageTestimonials();
			$where = array('testimonial_status' => '1');

			$GLOBALS['smarty']->assign('VIEW_TESTIMONIALS_ENABLED', $GLOBALS['language']->testimonials['testimonials_page_enabled']);
			$GLOBALS['gui']->addBreadcrumb($GLOBALS['language']->testimonials['testimonials_page_breadcrumb'], $GLOBALS['seo']->buildURL('testimonials'));

			if (($testimonials = $GLOBALS['db']->select('CubeCart_Testimonials', false, array('testimonial_status' => '1'),array('testimonial_order'=>'ASC'), $per_page, $page)) !== false) {

				if (($paginate = $GLOBALS['db']->select('CubeCart_Testimonials', false, $where)) !== false) {
					$GLOBALS['smarty']->assign('PAGINATION', $GLOBALS['db']->pagination(count($paginate), $per_page, $page));
				}

				if($GLOBALS['config']->get('config','sfws_testimonials_page_pagination') == 1){
					$GLOBALS['smarty']->assign('AJAX_PAGINATION', true);
				} else {
					$GLOBALS['smarty']->assign('TRADITIONAL_PAGINATION', true);
				}
				
				foreach ($testimonials as $testimonial) {
					$testimonial['testimonial_date'] = formatTime($testimonial['testimonial_date'],false,true);
					$vars['testimonials'][] = $testimonial;
				}

				$GLOBALS['smarty']->assign('TESTIMONIALS', $vars['testimonials']);

			}

			$GLOBALS['smarty']->assign('PAGE_SPLITS', $GLOBALS['gui']->perPageSplitsTestimonials());

		} else {

			$GLOBALS['smarty']->assign('VIEW_TESTIMONIALS_DISABLED', $GLOBALS['language']->testimonials['testimonials_page_disabled']);

		}
		$meta_description = $GLOBALS['config']->get('config','sfws_testimonials_page_seo_meta_description');
		$meta_keywords = $GLOBALS['config']->get('config','sfws_testimonials_page_seo_meta_keywords');
		$meta_title = $GLOBALS['config']->get('config','sfws_testimonials_page_seo_browser_title');
		$meta_data = array(
			'name'   => null,
			'path'   => null,
			'description' => (isset($meta_description)) ? $meta_description : '',
			'keywords'  => (isset($meta_keywords)) ? $meta_keywords : '',
			'title'   => (isset($meta_title)) ? $meta_title : '',
		);
		$GLOBALS['seo']->set_meta_data($meta_data);
		$content = $GLOBALS['smarty']->fetch('templates/content.testimonials.php');
		$GLOBALS['smarty']->assign('PAGE_CONTENT', $content);

	}

	/**
	 * Display add testimonial page
	 */
	private function _addtestimonial() {
		/* !Add Testimonial Form */
		if($GLOBALS['config']->get('config','sfws_testimonials_add_status') == 1){
			$GLOBALS['gui']->addBreadcrumb($GLOBALS['language']->testimonials['testimonials_add_breadcrumb'], $GLOBALS['seo']->buildURL('addtestimonial'));
			if (isset($_POST['add_testimonial'])) {
				$GLOBALS['smarty']->assign('MESSAGE', $_POST['add_testimonial']);
				// Validation
				$error = false;
				$record	= $_POST['add_testimonial'];
	// I DON'T WANT SHORT REQUIRED			$required	= array('customer_email', 'customer_name', 'testimonial_short', 'testimonial_long');

				$required	= array('customer_email', 'customer_name', 'testimonial_long');
				foreach ($_POST['add_testimonial'] as $key => $value) {
					if (in_array($key, $required) && empty($value)) {
						$GLOBALS['gui']->setError($GLOBALS['testimonials']->common['testimonials_add_fields_required_error']);
						$error = true;
						break;
					}
				}
				if (!filter_var($_POST['add_testimonial']['customer_email'], FILTER_VALIDATE_EMAIL)) {
					$GLOBALS['gui']->setError($GLOBALS['language']->common['error_email_invalid']);
					$error = true;
				}
				...
				$record['testimonial_date']	= time();
				$record['testimonial_ip_address'] = get_ip_address();

				if (!$error) {
					if (($GLOBALS['db']->insert('CubeCart_Testimonials', $record)) !== false) {
						$GLOBALS['gui']->setNotify($GLOBALS['language']->testimonials['testimonials_add_success']);
						$mail					= Mailer::getInstance();
						$record['link'] 		= $GLOBALS['storeURL'].'/'.$GLOBALS['config']->get('config','adminFile').'?_g=testimonials&action=edit&testimonial_id='.$GLOBALS['db']->insertid();
						$content				= $mail->loadContent('admin.testimonial_added', $GLOBALS['language']->current(), $record);
						if (!empty($content)) {
							$mail->sendEmail($GLOBALS['config']->get('config', 'email_address'), $content);
						}
						httpredir('index.php');
					} else {
						$GLOBALS['gui']->setError($GLOBALS['language']->testimonials['testimonials_add_error']);
					}
				} else {
					foreach($_POST['add_testimonial'] as $key => $value) {
						$_POST['add_testimonial'][$key] = htmlspecialchars($value);
					}
					$GLOBALS['smarty']->assign('MESSAGE', $_POST['add_testimonial']);
					$GLOBALS['gui']->setError($GLOBALS['language']->testimonials['testimonials_add_error']);
				}
			...
			// Display form
			$GLOBALS['smarty']->assign('ADD_TESTIMONIAL_ENABLED', $GLOBALS['language']->testimonials['testimonials_add_enabled']);
			/*
			if ($GLOBALS['config']->get('config', 'recaptcha') && !$GLOBALS['session']->get('confirmed','recaptcha')) {
				$GLOBALS['smarty']->assign('LANG_RECAPTCHA', array(
					'reload_words' => sprintf($GLOBALS['language']->recaptcha['reload_words'], 'javascript:Recaptcha.reload()', "javascript:Recaptcha.switch_type('audio')"),
			...
			*/
			...
			$meta_description = $GLOBALS['config']->get('config','sfws_testimonials_add_seo_meta_description');
			$meta_keywords = $GLOBALS['config']->get('config','sfws_testimonials_add_seo_meta_keywords');
			$meta_title = $GLOBALS['config']->get('config','sfws_testimonials_add_seo_browser_title');
			$meta_data = array(
				'name'   => null,
				'path'   => null,
				'description' => (isset($meta_description)) ? $meta_description : '',
			...
			$GLOBALS['seo']->set_meta_data($meta_data);
			$content = $GLOBALS['smarty']->fetch('templates/content.add_testimonial.php');
			$GLOBALS['smarty']->assign('PAGE_CONTENT', $content);
		} else {
			$GLOBALS['smarty']->assign('ADD_TESTIMONIAL_DISABLED', $GLOBALS['language']->testimonials['testimonials_add_disabled']);
			$meta_description = $GLOBALS['config']->get('config','sfws_testimonials_add_seo_meta_description');
			$meta_keywords = $GLOBALS['config']->get('config','sfws_testimonials_add_seo_meta_keywords');
			$meta_title = $GLOBALS['config']->get('config','sfws_testimonials_add_seo_browser_title');
			$meta_data = array(
	...
	private function _contact() {
		// Contact Form
/* Cubehelper Contact Us
$cubehelper_contact_us_form = $GLOBALS['config']->get('Custom_Contact_Us');
       if( $cubehelper_contact_us_form && $cubehelper_contact_us_form['status']) {
             foreach ($GLOBALS['hooks']->load('class.gui.cubehelper_custom_contact_us_post_processing') as $hook) include $hook;
	     foreach ($GLOBALS['hooks']->load('class.gui.cubehelper_custom_contact_us_pre_processing') as $hook) include $hook;

       } else {
end Cubehelper Contact Us */
...
//  } //Cubehelper Contact Us BRACE ADDED ABOVE
	}
...
//end Sequential Order Number tweak for Lookup Order
	...
	// SEQUENTIAL VERSION
		...
		// END SEQUENTIAL VERSION
			...
			/* 608 version before Sequential Order tweak
			 ...
			 */
//Sequential Order Tweak
   if (isset($_POST['cart_order_id']) && isset($_POST['email']) && filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)){
//end of Sequential Order Tweak
