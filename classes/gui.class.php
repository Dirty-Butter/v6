...
class GUI {
	...
	public function displayCommon() {
		if (!CC_IN_ADMIN) {
			...
			if($GLOBALS['config']->get('config','sfws_site_doc_manager_status') == 0){
			$this->_displayDocuments();
			}
			if($GLOBALS['config']->get('config','sfws_site_doc_manager_status') == 1){
				if($GLOBALS['config']->get('config','sfws_site_doc_manager_status_header') == 1){
					$this->_displayDocumentsHeader();
				}
				if($GLOBALS['config']->get('config','sfws_site_doc_manager_status_footer') == 1){
					$this->_displayDocumentsFooter();
				}
				if($GLOBALS['config']->get('config','sfws_site_doc_manager_status_sidebox1') == 1){
					$this->_displayDocumentsSideBox1();
				}
				if($GLOBALS['config']->get('config','sfws_site_doc_manager_status_sidebox2') == 1){
					$this->_displayDocumentsSideBox2();
				}
			}
			...
			if($GLOBALS['config']->get('config','sfws_testimonials_sidebox_status') == 1){
				$this->_displayRandomSiteTestimonial();
			}
			// END SEMPERFI TESTIMONIALS 
			// BEGINNING SEMPERFI SCRIPTURES   
			if($GLOBALS['config']->get('config','sfws_scriptures_footer_status') == 1){
				$this->_displayRandomScriptureFooter();
			}
			if($GLOBALS['config']->get('config','sfws_scriptures_sidebox_status') == 1){
				$this->_displayRandomScriptureSidebox();
			}
			// END SEMPERFI SCRIPTURES  
			// Start SemperFi CONTENT SLIDER Modification
			if($GLOBALS['config']->get('config','sfws_content_slider_status') == 1){
				$this->_displayContentSlider();
			}
	 ...
	 */
	private function _displayDocumentsHeader() {
		$varsHeader = array();
		if ($GLOBALS['user']->is()) {
			if (($docsHeader = $GLOBALS['db']->select('CubeCart_documents', false, array('doc_parent_id' => '0', 'doc_status' => '1', 'doc_header_status' => '1'),'`doc_order` ASC')) !== false) {
				foreach ($docsHeader as $docHeader) {
					if ($docHeader['doc_home']) {
						continue;
					}
					if (empty($docHeader['doc_url'])) {
						$docHeader['doc_url'] = $GLOBALS['seo']->buildURL('doc', $docHeader['doc_id']);
					}
					if ($GLOBALS['language']->translateDocument($docHeader)) {
						$varsHeader[] = $docHeader;
					}
				}
			}
		} else {
			if (($docsHeader = $GLOBALS['db']->select('CubeCart_documents', false, array('doc_parent_id' => '0', 'doc_status' => '1', 'doc_header_status' => '1', 'doc_access' => '0'),'`doc_order` ASC')) !== false) {
				foreach ($docsHeader as $docHeader) {
					if ($docHeader['doc_home']) {
						continue;
					}
		...
		foreach ($GLOBALS['hooks']->load('class.gui.display_documents') as $hook) include $hook;
		$GLOBALS['smarty']->assign('DOCUMENTS_HEADER', $varsHeader);
		$content = $GLOBALS['smarty']->fetch('templates/box.documents_header.php');
		$GLOBALS['smarty']->assign('SITE_DOCS_HEADER', $content);
	}

	/**
	 * Display footer documents
	 */
	private function _displayDocumentsFooter() {
		$varsFooter = array();
		if ($GLOBALS['user']->is()) {
			if (($docsFooter = $GLOBALS['db']->select('CubeCart_documents', false, array('doc_parent_id' => '0', 'doc_status' => '1', 'doc_footer_status' => '1'),'`doc_order` ASC')) !== false) {
				foreach ($docsFooter as $docFooter) {
					if ($docFooter['doc_home']) {
						continue;
					}
					if (empty($docFooter['doc_url'])) {
						$docFooter['doc_url'] = $GLOBALS['seo']->buildURL('doc', $docFooter['doc_id']);
					}
					if ($GLOBALS['language']->translateDocument($docFooter)) {
						$varsFooter[] = $docFooter;
					}
				}
			}
		} else {
			if (($docsFooter = $GLOBALS['db']->select('CubeCart_documents', false, array('doc_parent_id' => '0', 'doc_status' => '1', 'doc_footer_status' => '1', 'doc_access' => '0'),'`doc_order` ASC')) !== false) {
				foreach ($docsFooter as $docFooter) {
					if ($docFooter['doc_home']) {
						continue;
					}
		...
		foreach ($GLOBALS['hooks']->load('class.gui.display_documents') as $hook) include $hook;
		$GLOBALS['smarty']->assign('DOCUMENTS_FOOTER', $varsFooter);
		$content = $GLOBALS['smarty']->fetch('templates/box.documents_footer.php');
		$GLOBALS['smarty']->assign('SITE_DOCS_FOOTER', $content);
	}

	/**
	 * Display side box 1 documents
	 */
	private function _displayDocumentsSideBox1() {
		$varsSideBox1 = array();
		if ($GLOBALS['user']->is()) {
			if (($docsSideBox1 = $GLOBALS['db']->select('CubeCart_documents', false, array('doc_parent_id' => '0', 'doc_status' => '1', 'doc_sidebox01_status' => '1'),'`doc_order` ASC')) !== false) {
				foreach ($docsSideBox1 as $docSideBox1) {
					if ($docSideBox1['doc_home']) {
						continue;
					}
					if (empty($docSideBox1['doc_url'])) {
						$docSideBox1['doc_url'] = $GLOBALS['seo']->buildURL('doc', $docSideBox1['doc_id']);
					}
					if ($GLOBALS['language']->translateDocument($docSideBox1)) {
						$varsSideBox1[] = $docSideBox1;
					}
				}
			}
		} else {
			if (($docsSideBox1 = $GLOBALS['db']->select('CubeCart_documents', false, array('doc_parent_id' => '0', 'doc_status' => '1', 'doc_sidebox01_status' => '1', 'doc_access' => '0'),'`doc_order` ASC')) !== false) {
				foreach ($docsSideBox1 as $docSideBox1) {
					if ($docSideBox1['doc_home']) {
						continue;
					}
		...
		foreach ($GLOBALS['hooks']->load('class.gui.display_documents') as $hook) include $hook;
		$GLOBALS['smarty']->assign('DOCUMENTS_SIDEBOX1', $varsSideBox1);
		$content = $GLOBALS['smarty']->fetch('templates/box.documents_sidebox1.php');
		$GLOBALS['smarty']->assign('SITE_DOCS_SIDEBOX1', $content);
	}

	/**
	 * Display side box 2 documents
	 */
	private function _displayDocumentsSideBox2() {
		$varsSideBox2 = array();
		if ($GLOBALS['user']->is()) {
			if (($docsSideBox2 = $GLOBALS['db']->select('CubeCart_documents', false, array('doc_parent_id' => '0', 'doc_status' => '1', 'doc_sidebox02_status' => '1'),'`doc_order` ASC')) !== false) {
				foreach ($docsSideBox2 as $docSideBox2) {
					if ($docSideBox2['doc_home']) {
						continue;
					}
					if (empty($docSideBox2['doc_url'])) {
						$docSideBox2['doc_url'] = $GLOBALS['seo']->buildURL('doc', $docSideBox2['doc_id']);
					}
					if ($GLOBALS['language']->translateDocument($docSideBox2)) {
						$varsSideBox2[] = $docSideBox2;
					}
				}
			}
		} else {
			if (($docsSideBox2 = $GLOBALS['db']->select('CubeCart_documents', false, array('doc_parent_id' => '0', 'doc_status' => '1', 'doc_sidebox02_status' => '1', 'doc_access' => '0'),'`doc_order` ASC')) !== false) {
				foreach ($docsSideBox2 as $docSideBox2) {
					if ($docSideBox2['doc_home']) {
						continue;
					}
		...
		foreach ($GLOBALS['hooks']->load('class.gui.display_documents') as $hook) include $hook;
		$GLOBALS['smarty']->assign('DOCUMENTS_SIDEBOX2', $varsSideBox2);
		$content = $GLOBALS['smarty']->fetch('templates/box.documents_sidebox2.php');
		$GLOBALS['smarty']->assign('SITE_DOCS_SIDEBOX2', $content);
	}
	// End SemperFi DOCUMENT MANAGER Modification
	...
	// Start SemperFi CONTENT SLIDER Modification
	/**
	 * Display content slider box
	 */
	private function _displayContentSlider() {
		$content_slider_status = $GLOBALS['config']->get('config', 'sfws_content_slider_status');
		$content_slider_display_location = $GLOBALS['config']->get('config', 'sfws_content_slider_display_location');
		if($content_slider_status && !empty($content_slider_display_location)){
			$slides = array();
			$get_slides = $GLOBALS['db']->select('CubeCart_Content_Slider', false,  array('slide_status' => '1'), array('slide_order' => 'ASC'), '1000');
			if($get_slides){
				// Set Content Slider Variables
				$GLOBALS['smarty']->assign('CS_AUTO_SLIDE', $GLOBALS['config']->get('config', 'sfws_content_slider_auto_slide'));
				$GLOBALS['smarty']->assign('CS_AUTO_SLIDE_DIRECTION', $GLOBALS['config']->get('config', 'sfws_content_slider_auto_slide_direction'));
				$GLOBALS['smarty']->assign('CS_AUTO_SLIDE_INTERVAL', $GLOBALS['config']->get('config', 'sfws_content_slider_auto_slide_interval'));
				$GLOBALS['smarty']->assign('CS_FORCE_AUTO_SLIDE', $GLOBALS['config']->get('config', 'sfws_content_slider_force_auto_slide'));
				$GLOBALS['smarty']->assign('CS_PAUSE_ON_HOVER', $GLOBALS['config']->get('config', 'sfws_content_slider_pause_on_hover'));
				$GLOBALS['smarty']->assign('CS_AUTO_HEIGHT', $GLOBALS['config']->get('config', 'sfws_content_slider_auto_height'));
				$GLOBALS['smarty']->assign('CS_HEIGHT_EASE_DURATION', $GLOBALS['config']->get('config', 'sfws_content_slider_height_ease_duration'));
				$GLOBALS['smarty']->assign('CS_HEIGHT_EASE_FUNCTION', $GLOBALS['config']->get('config', 'sfws_content_slider_height_ease_function'));
				$GLOBALS['smarty']->assign('CS_SLIDE_EASE_DURATION', $GLOBALS['config']->get('config', 'sfws_content_slider_slide_ease_duration'));
				$GLOBALS['smarty']->assign('CS_SLIDE_EASE_FUNCTION', $GLOBALS['config']->get('config', 'sfws_content_slider_slide_ease_function'));
				$GLOBALS['smarty']->assign('CS_FADE_IN_DURATION', $GLOBALS['config']->get('config', 'sfws_content_slider_fade_in_duration'));
				$GLOBALS['smarty']->assign('CS_FADE_OUT_DURATION', $GLOBALS['config']->get('config', 'sfws_content_slider_fade_out_duration'));
				$GLOBALS['smarty']->assign('CS_DYNAMIC_ARROWS', $GLOBALS['config']->get('config', 'sfws_content_slider_dynamic_arrows'));
				$GLOBALS['smarty']->assign('CS_DYNAMIC_ARROWS_GRAPHICAL', $GLOBALS['config']->get('config', 'sfws_content_slider_dynamic_arrows_graphical'));
				$GLOBALS['smarty']->assign('CS_DYNAMIC_ARROW_LEFT_TEXT', $GLOBALS['config']->get('config', 'sfws_content_slider_dynamic_arrow_left_text'));
				$GLOBALS['smarty']->assign('CS_DYNAMIC_ARROW_RIGHT_TEXT', $GLOBALS['config']->get('config', 'sfws_content_slider_dynamic_arrow_right_text'));
				$GLOBALS['smarty']->assign('CS_HIDE_SIDE_ARROWS', $GLOBALS['config']->get('config', 'sfws_content_slider_hide_side_arrows'));
				$GLOBALS['smarty']->assign('CS_HIDE_SIDE_ARROWS_DURATION', $GLOBALS['config']->get('config', 'sfws_content_slider_hide_side_arrows_duration'));
				$GLOBALS['smarty']->assign('CS_HOVER_ARROWS', $GLOBALS['config']->get('config', 'sfws_content_slider_hover_arrows'));
				$GLOBALS['smarty']->assign('CS_HOVER_ARROWS_DURATION', $GLOBALS['config']->get('config', 'sfws_content_slider_hover_arrows_duration'));
				$GLOBALS['smarty']->assign('CS_DYNAMIC_TABS', $GLOBALS['config']->get('config', 'sfws_content_slider_dynamic_tabs'));
				$GLOBALS['smarty']->assign('CS_DYNAMIC_TABS_ALIGN', $GLOBALS['config']->get('config', 'sfws_content_slider_dynamic_tabs_align'));
				$GLOBALS['smarty']->assign('CS_DYNAMIC_TABS_POSITION', $GLOBALS['config']->get('config', 'sfws_content_slider_dynamic_tabs_position'));
				$GLOBALS['smarty']->assign('CS_INCLUDE_TITLE', $GLOBALS['config']->get('config', 'sfws_content_slider_include_title'));
				$GLOBALS['smarty']->assign('CS_RESPONSIVE', $GLOBALS['config']->get('config', 'sfws_content_slider_responsive'));
				$GLOBALS['smarty']->assign('CS_MOBILE_NAVIGATION', $GLOBALS['config']->get('config', 'sfws_content_slider_mobile_navigation'));
				$GLOBALS['smarty']->assign('CS_MOBILE_NAV_DEFAULT_TEXT', $GLOBALS['config']->get('config', 'sfws_content_slider_mobile_nav_default_text'));
				$GLOBALS['smarty']->assign('CS_HIDE_ARROWS_WHEN_MOBILE', $GLOBALS['config']->get('config', 'sfws_content_slider_hide_arrows_when_mobile'));
				$GLOBALS['smarty']->assign('CS_HIDE_ARROWS_THRESHOLD', $GLOBALS['config']->get('config', 'sfws_content_slider_hide_arrows_threshold'));
				$GLOBALS['smarty']->assign('CS_SWIPE', $GLOBALS['config']->get('config', 'sfws_content_slider_swipe'));
				// Set Slides
				foreach ($get_slides as $get_slide) {
					$slides[] = $get_slide;
				}
				// Send It All To The Template
				$GLOBALS['smarty']->assign('CONTENT_SLIDER_STATUS', true);
				$GLOBALS['smarty']->assign('CONTENT_SLIDER_DISPLAY_LOCATION', $content_slider_display_location);
				$GLOBALS['smarty']->assign('SLIDES', $slides);
				$content = $GLOBALS['smarty']->fetch('templates/box.contentslider.php');
				$GLOBALS['smarty']->assign('SFWS_CONTENT_SLIDER', $content);
			}
		}
	}
	// End SemperFi CONTENT SLIDER Modification
...
// BEGINNING SEMPERFI TESTIMONIALS 
	 ...
	 * @return int
	 */
	public function itemsPerPageTestimonials($list_id = 'testimonials', $page_key = 'perpage') {
		...
		if (is_numeric($GLOBALS['config']->get('config', 'sfws_testimonials_page_amount'))) {
			return $GLOBALS['config']->get('config', 'sfws_testimonials_page_amount');
		} else { // Last ditch ..
			return 10;
		} 
	}
	...
	/**
	 * Get testimonials page splits
	 *
	 * @param string $list_id
	 * @param string $page_key
	 * @return int
	 */
	public function perPageSplitsTestimonials($list_id = 'testimonials', $page_key = 'perpage') {
		if(isset($this->_skin_data['layout'][$list_id]['perpage'])) {
			foreach($this->_skin_data['layout'][$list_id]['perpage'] as $amount => $default) {
				if(!isset($_GET[$page_key]) && $default) {
					$selected = true;
	...
	/**
	 * Display random site testimonial box // I ADDED TITLE
	 */
	private function _displayRandomSiteTestimonial() {
		if (($query = $GLOBALS['db']->select('CubeCart_Testimonials', false, array('testimonial_status' => '1'), 'RAND()', '1', false, false)) !== false) {
			for($i=0, $max=count($query); $i<$max; $i++) {
				$testimonial = $query[$i];
			}
			$testimonial['testimonial_title'] = $testimonial['testimonial_title']; 
			$testimonial['customer_name'] = $testimonial['customer_name'];
			$testimonial['testimonial_short'] = (strlen($testimonial['testimonial_short']) > $GLOBALS['config']->get('config', 'sfws_testimonials_sidebox_character_length')) ? substr(strip_tags($testimonial['testimonial_short']), 0, $GLOBALS['config']->get('config', 'sfws_testimonials_sidebox_character_length')).'&hellip;' : $testimonial['testimonial_short'];
			$GLOBALS['smarty']->assign('testimonial', $testimonial);
			$GLOBALS['smarty']->assign('ADD_TESTIMONIAL_URL', $GLOBALS['seo']->buildURL('addtestimonial'));
			$GLOBALS['smarty']->assign('VIEW_TESTIMONIALS_URL', $GLOBALS['seo']->buildURL('testimonials'));
			$content = $GLOBALS['smarty']->fetch('templates/box.testimonial.php');
			$GLOBALS['smarty']->assign('RANDOM_SITE_TESTIMONIAL', $content);
		}
	}
	// END SEMPERFI TESTIMONIALS 
	...
	/**
	 * Display random scripture footer
	 */
	private function _displayRandomScriptureFooter() {
		if (($query = $GLOBALS['db']->select('CubeCart_scriptures', false, false, 'RAND()', '1', false, false)) !== false) {
			for($i=0, $max=count($query); $i<$max; $i++) {
				$scripture_footer = $query[$i];
			}
			$scripture_footer['scripture_verse'] = $scripture_footer['scripture_verse'];
			$GLOBALS['smarty']->assign('scripture_footer', $scripture_footer);
			$content = $GLOBALS['smarty']->fetch('templates/box.scripture-footer.php');
			$GLOBALS['smarty']->assign('RANDOM_SCRIPTURE_FOOTER', $content);
		}
	}
	/**
	 * Display random scripture sidebox
	 */
	private function _displayRandomScriptureSidebox() {
		if (($query = $GLOBALS['db']->select('CubeCart_scriptures', false, false, 'RAND()', '1', false, false)) !== false) {
			for($i=0, $max=count($query); $i<$max; $i++) {
				$scripture_sidebox = $query[$i];
			}
			$scripture_sidebox['scripture_verse'] = $scripture_sidebox['scripture_verse'];
			$GLOBALS['smarty']->assign('scripture_sidebox', $scripture_sidebox);
			$content = $GLOBALS['smarty']->fetch('templates/box.scripture-sidebox.php');
			$GLOBALS['smarty']->assign('RANDOM_SCRIPTURE_SIDEBOX', $content);
		}
	}
	// End SEMPERFI SCRIPTURES
	/**
