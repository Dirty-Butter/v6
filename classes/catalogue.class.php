...
/* END  BSMITHER PAGE=ALL METADATA FIX */
		...
		// Start SemperFi DOCUMENT MANAGER Modification
			if($GLOBALS['config']->get('config','sfws_site_doc_manager_status') == 1 && $GLOBALS['config']->get('config','sfws_site_doc_manager_status_categories') == 1 && is_numeric($_GET['cat_id'])){
			$varsCategory = array();
			if ($GLOBALS['user']->is()) {
				$docsCategoryQuery = sprintf("SELECT * FROM `%1\$sCubeCart_documents` WHERE doc_parent_id = 0 AND doc_status = 1 AND (doc_category_assign = 'ALL' OR doc_category_assign = '".$_GET['cat_id']."') ORDER BY doc_order", $GLOBALS['config']->get('config', 'dbprefix'));
			} else {
				$docsCategoryQuery = sprintf("SELECT * FROM `%1\$sCubeCart_documents` WHERE doc_parent_id = 0 AND doc_status = 1 AND (doc_category_assign = 'ALL' OR doc_category_assign = '".$_GET['cat_id']."') AND doc_access = 0 ORDER BY doc_order", $GLOBALS['config']->get('config', 'dbprefix'));
			}
			if (($docsCategory = $GLOBALS['db']->query($docsCategoryQuery)) !== false) {
				foreach ($docsCategory as $docCategory) {
					if ($docCategory['doc_home']) {
						continue;
					}
					if (empty($docCategory['doc_url'])) {
						$docCategory['doc_url'] = $GLOBALS['seo']->buildURL('doc', $docCategory['doc_id']);
					}
					if ($GLOBALS['language']->translateDocument($docCategory)) {
						$varsCategory[] = $docCategory;
					}
				}
			} 
			$GLOBALS['smarty']->assign('DOCUMENTS_CATEGORY', $varsCategory);
		}
	...
	public function displayProduct($product = false, $popularity = false) {
		if (isset($product) && is_numeric($product)) {
			...
			// Start SemperFi DOCUMENT MANAGER  Modification
			if($GLOBALS['config']->get('config','sfws_site_doc_manager_status') == 1 && $GLOBALS['config']->get('config','sfws_site_doc_manager_status_products') == 1){
				$varsProduct = array();
				if ($GLOBALS['user']->is()) {
					$docsProductQuery = sprintf("SELECT * FROM `%1\$sCubeCart_documents` WHERE doc_parent_id = 0 AND doc_status = 1 AND (doc_product_assign = 'ALL' OR doc_product_assign = '".$product['product_id']."') ORDER BY doc_order", $GLOBALS['config']->get('config', 'dbprefix'));
				} else {
					$docsProductQuery = sprintf("SELECT * FROM `%1\$sCubeCart_documents` WHERE doc_parent_id = 0 AND doc_status = 1 AND (doc_product_assign = 'ALL' OR doc_product_assign = '".$product['product_id']."') AND doc_access = 0 ORDER BY doc_order", $GLOBALS['config']->get('config', 'dbprefix'));
				}
				if (($docsProduct = $GLOBALS['db']->query($docsProductQuery)) !== false) {
					foreach ($docsProduct as $docProduct) {
						if ($docProduct['doc_home']) {
							continue;
						}
						if (empty($docProduct['doc_url'])) {
							$docProduct['doc_url'] = $GLOBALS['seo']->buildURL('doc', $docProduct['doc_id']);
						}
						if ($GLOBALS['language']->translateDocument($docProduct)) {
							$varsProduct[] = $docProduct;
						}
					}
				} 
				$GLOBALS['smarty']->assign('DOCUMENTS_PRODUCT', $varsProduct);
			}
			// End SemperFi DOCUMENT MANAGER Modification

			// Start SemperFi ASKABOUT Modification
			if($GLOBALS['config']->get('config','ask_about_a_product_status') == 1){
				// Set The Out Of Stock Variable
				if ((bool)$product['use_stock_level'] && $product['stock_level'] <= 0) {
					$GLOBALS['smarty']->assign('ASKABOUT_PRODUCT_OUT_OF_STOCK', true);
				} else {
					$GLOBALS['smarty']->assign('ASKABOUT_PRODUCT_OUT_OF_STOCK', false);
				}
				// Set The Price Variable
				$ask_price = substr($product['price'], 1);
				$ask_price = str_replace(",", "", $ask_price);
				$ask_sale_price = substr($product['sale_price'], 1);
				$ask_sale_price = str_replace(",", "", $ask_sale_price);
				if($ask_sale_price > 34){
					$GLOBALS['smarty']->assign('ASKABOUT_PRODUCT_PRICING', true);
				} else if($ask_price > 34){
					$GLOBALS['smarty']->assign('ASKABOUT_PRODUCT_PRICING', true);
				} else {
					$GLOBALS['smarty']->assign('ASKABOUT_PRODUCT_PRICING', false);
				}
				// Set The URL
				$askabout_url = $GLOBALS['seo']->buildURL('askabout', $product['product_id']);
				$GLOBALS['smarty']->assign('ASKABOUT_PRODUCT_URL', $askabout_url);
			}
	 ...
	 * @return bool
	 */
	// ORIGINAL B4 BSMITHER SEARCH HACK CHANGE TO LIKE INSTEAD OF FULLTEXT public function searchCatalogue($search_data = null, $page = 1, $per_page = 10, $search_mode = 'fulltext') {
...
/*		ORIGINAL B4 BSMITHER SEARCH HACK
