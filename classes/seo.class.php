...
class SEO {
	 ...
	 * @var array of strings
	 */
	// BEGINNING SEMPERFI TESTIMONIALS  PLUSH
	// Old Code:
...
/**
     ...
     * @var array
     */
    private $_robot_disallow_types   = array(); // edit to suit
    /**
...
// END BSMITHER ROBOTS SITEMAP HACK 
  ...
  // Get last part of path  BSMITHER HACK FOR SORT
  ...
  $path = array_pop($cat_path_parts);
				...
				case 'askabout':
					// check its not been made already
					if (($existing = $GLOBALS['db']->select('CubeCart_seo_urls', 'path', array('type' => 'askabout', 'item_id' => $id))) !== false) {
						$path = $existing[0]['path'];
					} else {
						$askabout = $GLOBALS['db']->select('CubeCart_inventory', array('name','product_id'), array('product_id' => $id));
						$path = $askabout[0]['name']."-askabout-".$askabout[0]['product_id'];
					}
					break;
...
if( $GLOBALS['config']->has('config', 'robots_path') && file_exists($GLOBALS['config']->get('config', 'robots_path')) ) {
    ...
    foreach( $robot_directive_array as $line ) {
        if( preg_match('#Disallow:[\s]*/(.+)#i', $line, $matches) ) $this->_robot_disallows[] = str_replace('*', '.*', $matches[1]);
    }
}
// END BEMITHER ROBOTS SITEMAP HACK 
			...
			if ($results) {
				foreach ($results as $record) {
					...
					case 'document':
						...
						$key = 'doc_id';
						break;
// Start SemperFi Addition removed to prevent askabout page showing in sitemap
//						case 'askabout':
//							$id		= $record['product_id'];
//							$key	= 'product_id';
//							break;
...
// BEGINNING SEMPERFI TESTIMONIALS 
	...
	private function _sitemap_link($input, $updated = false, $type = false) {
		...
		//}
// BSMITHER ROBOTS SITEMAP revised 071215 
