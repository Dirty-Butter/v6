...
class Order {
	...
	const ORDER_FAILED		= 5;	# Fraudulent CHANGED TO UNPAID ORDER IN ADMIN LANGUAGES WITH APPROPRIATE EMAIL WORDING
		...
		if (($items = $GLOBALS['db']->select('CubeCart_order_inventory', false, array('cart_order_id' => $order_id), false, false, false, false)) !== false) {
			...
			foreach ($items as $item) {
				...
				if (($product = $GLOBALS['db']->select('CubeCart_inventory', array('stock_level'), array('product_id' => (int)$item['product_id'], 'use_stock_level' => 1), false, false, false, false)) !== false) {
					...
					case self::ORDER_DECLINED:
					case self::ORDER_FAILED:
## Order Failed is what I renamed and use as Unpaid Order so stock DOES need to be updated
							## Restore stock
							if ($item['stock_updated']) {
								$stock	= $stock+$item['quantity'];
								$update	= 0;
...
//Toucan Product Description PLUSH
				'prodDesc'			=> (isset($product['seo_meta_description'])) ? $product['seo_meta_description'] : '',
//end Toucan Product Description
				'hash'    => $hash,
