...
// New Code:
if (isset($_POST['terms']) || isset($_POST['home']) || isset($_POST['order']) || isset($_POST['status']) || isset($_POST['access']) || isset($_POST['header']) || isset($_POST['footer']) || isset($_POST['sidebox1']) || isset($_POST['sidebox2'])) {
// End SemperFi DOCUMENT MANAGER Modification
	if (Admin::getInstance()->permissions('documents', CC_PERM_EDIT)) {
		...
		if (isset($_POST['access']) && is_array($_POST['access'])) {
			...
			foreach ($_POST['access'] as $doc_id => $access) {
				if($GLOBALS['db']->update('CubeCart_documents', array('doc_access' => (int)$access), array('doc_id' => (int)$doc_id))) {
					$status_updated	= true;
				}
			}
		}
		...
		if (isset($_POST['header']) && is_array($_POST['header'])) {
			$status_updated = false;
			foreach ($_POST['header'] as $doc_id => $header) {
				if($GLOBALS['db']->update('CubeCart_documents', array('doc_header_status' => (int)$header), array('doc_id' => (int)$doc_id))) {
					$status_updated	= true;
				}
			}
		}
		...
		if (isset($_POST['footer']) && is_array($_POST['footer'])) {
			$status_updated = false;
			foreach ($_POST['footer'] as $doc_id => $footer) {
				if($GLOBALS['db']->update('CubeCart_documents', array('doc_footer_status' => (int)$footer), array('doc_id' => (int)$doc_id))) {
					$status_updated	= true;
				}
			}
		}
		## Set side box 1 status
		if (isset($_POST['sidebox1']) && is_array($_POST['sidebox1'])) {
			$status_updated = false;
			foreach ($_POST['sidebox1'] as $doc_id => $sidebox1) {
				if($GLOBALS['db']->update('CubeCart_documents', array('doc_sidebox01_status' => (int)$sidebox1), array('doc_id' => (int)$doc_id))) {
					$status_updated	= true;
				}
			}
		}
		## Set side box 2 status
		if (isset($_POST['sidebox2']) && is_array($_POST['sidebox2'])) {
			$status_updated = false;
			foreach ($_POST['sidebox2'] as $doc_id => $sidebox2) {
				if($GLOBALS['db']->update('CubeCart_documents', array('doc_sidebox02_status' => (int)$sidebox2), array('doc_id' => (int)$doc_id))) {
					$status_updated	= true;
				}
			}
		}
...
if (isset($_GET['action'])) {
	...
	if (($products = $GLOBALS['db']->select('CubeCart_inventory', false, false, array('name' => 'ASC'))) !== false) {
		$product['selected'] = ($document[0]['doc_product_assign'] == "ALL") ? ' selected="SELECTED"' : '';
		$product['product_id'] = "ALL";
		$product['name'] = "All Products";
		$smarty_data['list_products'][]	= $product;
		foreach ($products as $product) {
			$product['selected'] = ($product['product_id'] == $document[0]['doc_product_assign']) ? ' selected="SELECTED"' : '';
			$smarty_data['list_products'][]	= $product;
		}
		$GLOBALS['smarty']->assign('PRODUCTS', $smarty_data['list_products']);
	}
	if (($categories = $GLOBALS['db']->select('CubeCart_category', false, false, array('cat_name' => 'ASC'))) !== false) {
		$category['selected'] = ($document[0]['doc_product_assign'] == "ALL") ? ' selected="SELECTED"' : '';
		$category['cat_id'] = "ALL";
		$category['cat_name'] = "All Categories";
		$smarty_data['list_categories'][]	= $category;
		foreach ($categories as $category) {
			$category['selected'] = ($category['cat_id'] == $document[0]['doc_category_assign']) ? ' selected="SELECTED"' : '';
			$smarty_data['list_categories'][]	= $category;
		}
		$GLOBALS['smarty']->assign('CATEGORIES', $smarty_data['list_categories']);
	}
	// End SemperFi DOCUMENT MANAGER Modification
