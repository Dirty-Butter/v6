...
// Customers Online
// ORIGINAL $timeLimit = time()-1800;  // 30 minutes
$timeLimit = time()-86400;  // 24 HOURS

/* ORIGINALif (isset($_GET['bots']) && $_GET['bots']=='false') {
...
/* ORIGINAL 6.0.4 $query  = sprintf("SELECT S.*, C.first_name, C.last_name FROM %1\$sCubeCart_sessions AS S LEFT JOIN %1\$sCubeCart_customer AS C ON S.customer_id = C.customer_id WHERE ".$filter."S.session_last>".$timeLimit." ORDER BY S.session_last DESC", $glob['dbprefix']);
...
if (($results = $GLOBALS['db']->query($query)) !== false) {
   ...
   foreach ($results as $user) { $user_agentstring = $user['useragent'];
        ...
        $user['is_admin']  = ((int)$user['admin_id'] > 0) ? 1 : 0;
//END BSMITHER IGNORE BOTS SEE BELOW FOR MORE
