...
$email_types = array(
// BEGINNING SEMPERFI TESTIMONIALS  I ADDED TITLE
	'admin.testimonial_added' => array(
		'description'	=> $lang['email']['title_macro_admin_testimonial'],
		'macros' => array(
			'{$DATA.customer_name}' => $lang['testimonials']['email_macro_customer_name'],
			'{$DATA.customer_email}' => $lang['testimonials']['email_macro_customer_email'],
			'{$DATA.link}' => $lang['testimonials']['email_macro_link'],
'{$DATA.testimonial_title}' => $lang['testimonials']['email_macro_testimonial_title'],  
			'{$DATA.testimonial_short}' => $lang['testimonials']['email_macro_testimonial_short'],
			'{$DATA.testimonial_long}' => $lang['testimonials']['email_macro_testimonial_long'],
			'{$DATA.testimonial_ip_address}' => $lang['testimonials']['email_macro_testimonial_ip_address'],
		),
	),
	// END SEMPERFI TESTIMONIALS 
//Semper Fi ASKABOUT Addition
	'admin.ask_about_a_product' => array(
		'description'	=> $lang['email']['title_macro_ask_about_a_product'],
		'macros'		=> array (
			'{$DATA.sender_name}'	=> $lang['email']['macro_askabout_sender_name'],
			'{$DATA.sender_email}'	=> $lang['email']['macro_askabout_sender_email'],
			'{$DATA.sender_phone}'	=> $lang['email']['macro_askabout_sender_phone'],
			'{$DATA.product_name}'	=> $lang['email']['macro_askabout_product_name'],
			'{$DATA.question}'		=> $lang['email']['macro_askabout_customer_question'],
			'{$DATA.product_id}'	=> $lang['email']['macro_askabout_product_id'],
			'{$DATA.product_code}'	=> $lang['email']['macro_askabout_product_code'],
			'{$DATA.product_url}'	=> $lang['email']['macro_askabout_product_url'],
		),
	),
	...
	//Toucan ECHECK
	'admin.order_echeck' => array(
		'description'	=> $lang['email']['title_macro_admin_echeck'],
		'macros'	=> array (
			'{$SHIPPING.first_name}' => $lang['email']['macro_first_name_d'],
			'{$SHIPPING.last_name}' => $lang['email']['macro_last_name_d'],
			'{$SHIPPING.company_name}' => $lang['email']['macro_company_name_d'],
...
if (isset($_GET['action']) && isset($_GET['type'])) {
	...
	// Toucan - ECHECK Email Fix
					if(empty($data['content_type']) && !empty($_REQUEST['content_type'])){
						$data['content_type'] = $_REQUEST['content_type'];
					}
					//END ECHECK EMAIL FIX
