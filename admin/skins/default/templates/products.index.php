      ...
      <table width="100%">
         <thead>
            <tr>
			   ...
			   <!-- SALE PRICE TAB -->
			<th nowrap="nowrap">{$THEAD.sale_price}</th>		
				<!-- END SALE PRICE TAB -->
	  			<th nowrap="nowrap">{$THEAD.stock_level}</th>
				<!-- MY DISCOUNT TAB  -->
				<th nowrap="nowrap">{$THEAD.discount}</th>				
				<!-- END MY DISCOUNT TAB -->
...
{* MY 6.0.7 VERSION		  		<td align="center"><input type="hidden" name="status[{$product.product_id}]" id="status_{$product.product_id}" value="{$product.status}" class="toggle" /></td> *}
...
<!-- END MY DISCOUNT IMAGE -->
         ...
         <tfoot>
            <tr>
			   ...
			   <!-- CHANGE COLUMN SPAN FOR MY EXTRA TABS -->
               <td colspan="13">
   ...
   <div id="general" class="tab_content">
	  ...
	  <!-- MY BOX LENGTH  WITH FIXEDSYS FONT FROM LAYOUT.CSS-->
<div><label for="name">{$LANG.catalogue.product_name}</label><input name="name" id="name" class="textbox required" type="text"  size="65" value="{$PRODUCT.name}" ></div>
      ...
      <fieldset>
         ...
         <div>
<!-- BSMITHER SHOW TOYS AS DEFAULT GOOGLE CATEGORY -->
 <div><label for="google_cat_code">{$LANG.catalogue.product_google_category}</label><span>
            ...
            {/foreach}
               ...
               {/if}
<br><br>
<a href="//celebird-cloud.appspot.com/apps/celebird-taxonomy-app.html" target="_blank">Link to Google Taxonomy Picker</a>
<br><br>
 <!-- END BSMITHER SHOW TOYS AS DEFAULT GOOGLE CATEGORY -->
            </span>
         </div>
      </fieldset>
	  <!-- STORE LINK TO THIS PRODUCT -->
<br>	  	
...
{* SEO TITLE  AND URL WITH MY FONT AND 64 CHARACTER WIDTH *}
      <h3>{$LANG.settings.tab_seo}</h3>
      <fieldset>
         <legend>{$LANG.settings.title_seo_meta_data}</legend>
         <div><label for="seo_meta_title">{$LANG.settings.seo_meta_title}</label><span><input name="seo_meta_title" id="seo_meta_title" class="textbox" type="text"  size="64" value="{$PRODUCT.seo_meta_title}"></span></div>
         <div><label for="seo_path">{$LANG.settings.seo_path} *</label><span><input name="seo_path" id="seo_path" class="textbox"  size="70"  type="text"   value="{$PRODUCT.seo_path}"></span></div>
   ...
   {/if}
<!--My attempt to create Product Notes Tab-->
<div id="product_notes" class="tab_content">
<h3>Product Notes</h3>
<fieldset><legend>Notes</legend>
	<!-- <div><span><textarea name="product_notes" id="product_notes" class="textbox">{$PRODUCT.product_notes}</textarea></span></div> -->
<div><span><textarea name="product_notes" id="product_notes"<textarea class="textbox fck">{$PRODUCT.product_notes}</textarea></span></div>
</fieldset>
</div>
<!--end attempt-->
...
{* SEO TITLE AND URL BOXES WITH FIXSYS FONT FROM MY LAYOUT.CSS ADDITIONS *}
   <div id="seo" class="tab_content">
      <h3>{$LANG.settings.title_seo}</h3>
      <fieldset>
         <legend>{$LANG.settings.title_seo_meta_data}</legend>
         <div><label for="seo_meta_title">{$LANG.settings.seo_meta_title}</label><span><input name="translate[seo_meta_title]" id="seo_meta_title" class="textbox" type="text" size="64" value="{$TRANS.seo_meta_title}"></span></div>
         <div><label for="seo_path">{$LANG.settings.seo_path}</label><span><input name="seo_path" id="seo_path" class="textbox" type="text" font-style="FixedSys" size="70" value="{$TRANS.seo_path}"></span></div>
