...
<td>{$THEAD.customer_notes}</td> <!--  MY CUSTOMER NOTES -->
		  <td>&nbsp;</td>
		</tr>
	  </thead>
	  <tbody>
		...
		<tr>
		  ...
		  <!-- MY CUSTOMER NOTES -->
{if !empty($customer.customer_notes)}
<td><a href="?_g=customers&sort%5Bregistered%5D=DESC&action=edit&customer_id={$customer.customer_id}#customer_notes"><img src="{$STORE_URL}/{$SKIN_VARS.admin_folder}/skins/default/images/note.png" alt="Notes"></a></td>
{/if}		
 <!-- END MY CUSTOMER NOTES -->
		</tr>
		{foreachelse}
		<tr>
  ...
  {if isset($DISPLAY_CUSTOMER_FORM)}
	...
	<h3>{$ADD_EDIT_CUSTOMER}</h3>
	  ...
	  <fieldset><legend>{$LANG.account.contact_details}</legend>
		<div><label for="cust-email">{$LANG.common.email}</label><span><input type="text" name="customer[email]" id="cust-email" value="{$CUSTOMER.email}" class="textbox"></span>
<!-- MY EMAIL ADDRESS -->
&nbsp;&nbsp;<a href="mailto:{$CUSTOMER.email}">Email</a> - Dirty Butter Plush Animal Shoppe Registration</div>
<!-- MY EMAIL ADDRESS -->
...
<!-- My attempt to create Customer Notes Tab -->
<div id="customer_notes" class="tab_content">
<h3>Customer Notes</h3>
<fieldset><legend>Notes</legend>
	<div><span><textarea name="customer[customer_notes]" id="customer_notes"<textarea class="textbox fck">{$CUSTOMER.customer_notes}</textarea></span></div>
</fieldset>
</div>
<!-- end -->
	{include file='templates/element.hook_form_content.php'}
