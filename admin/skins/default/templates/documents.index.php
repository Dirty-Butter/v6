...
<form action="{$VAL_SELF}" method="post">
	...
	<table>
	  <thead>
		<tr>
		  ...
		  <td>{$LANG.documents.document_restricted}</td>
		  <td>{$LANG.documents.document_location_header}</td>
		  <td>{$LANG.documents.document_location_footer}</td>
		  <td>{$LANG.documents.document_location_side_box_1}</td>
		  <td>{$LANG.documents.document_location_side_box_2}</td>
	  ...
	  {foreach from=$DOCUMENTS item=document}
		<tr>
		  ...
		  <!-- Start SemperFi DOCUMENT MANAGER Modification -->
		  <td align="center">
			<input type="hidden" id="access-{$document.doc_id}" name="access[{$document.doc_id}]" value="{$document.doc_access}" class="toggle" />	  
		  </td>
		  <td align="center">
			<input type="hidden" id="header-{$document.doc_id}" name="header[{$document.doc_id}]" value="{$document.doc_header_status}" class="toggle" />
		  </td>
		  <td align="center">
			<input type="hidden" id="footer-{$document.doc_id}" name="footer[{$document.doc_id}]" value="{$document.doc_footer_status}" class="toggle" /> 
		  </td>
		  <td align="center">
			<input type="hidden" id="sidebox1-{$document.doc_id}" name="sidebox1[{$document.doc_id}]" value="{$document.doc_sidebox01_status}" class="toggle" /> 
		  </td>
		  <td align="center">
			<input type="hidden" id="sidebox2-{$document.doc_id}" name="sidebox2[{$document.doc_id}]" value="{$document.doc_sidebox02_status}" class="toggle" /> 
		  </td>
		  <!-- End SemperFi DOCUMENT MANAGER Modification -->
  ...
  <div id="general" class="tab_content">
	...
	<fieldset><legend>{$LANG.common.general}</legend>
	  ...
	  <!-- Start SemperFi DOCUMENT MANAGER Modification -->
	  <div><label for="doc_access">{$LANG.documents.document_restricted}</label><span><input type="hidden" id="doc_access" name="document[doc_access]" value="{$DOCUMENT.doc_access}" class="toggle" /></span></div>
	  <div><label for="doc_header_status">{$LANG.documents.document_location_header}</label><span><input type="hidden" id="doc_header_status" name="document[doc_header_status]" value="{$DOCUMENT.doc_header_status}" class="toggle" /></span></div>
	  <div><label for="doc_footer_status">{$LANG.documents.document_location_footer}</label><span><input type="hidden" id="doc_footer_status" name="document[doc_footer_status]" value="{$DOCUMENT.doc_footer_status}" class="toggle" /></span></div>
	  <div><label for="doc_sidebox01_status">{$LANG.documents.document_location_side_box_1}</label><span><input type="hidden" id="doc_sidebox01_status" name="document[doc_sidebox01_status]" value="{$DOCUMENT.doc_sidebox01_status}" class="toggle" /></span></div>
	  <div><label for="doc_sidebox02_status">{$LANG.documents.document_location_side_box_2}</label><span><input type="hidden" id="doc_sidebox02_status" name="document[doc_sidebox02_status]" value="{$DOCUMENT.doc_sidebox02_status}" class="toggle" /></span></div>
	  <div><label for="doc_product_assign">{$LANG.documents.document_product_assignment}</label><span><select name="document[doc_product_assign]" id="doc_product_assign" class="textbox">
	  <option value="">{$LANG.form.none}</option>
		{foreach from=$PRODUCTS item=product}<option value="{$product.product_id}"{$product.selected}>{$product.name}</option>{/foreach}
	  </select></span></div>
	  <div><label for="doc_category_assign">{$LANG.documents.document_category_assignment}</label><span><select name="document[doc_category_assign]" id="doc_category_assign" class="textbox">
	  <option value="">{$LANG.form.none}</option>
		{foreach from=$CATEGORIES item=category}<option value="{$category.cat_id}"{$category.selected}>{$category.cat_name}</option>{/foreach}
	  </select></span></div>
	  <!-- End SemperFi DOCUMENT MANAGER Modification -->
